...
// Função para inserir os produtos em pequenos lotes para evitar timeout
async function insertProductsBatch(products, categoryId) {
  const batchSize = 5; // Tamanho do lote
  let successCount = 0;
  let errorCount = 0;
  
  // Dividir em lotes pequenos
  for (let i = 0; i < products.length; i += batchSize) {
    ...
    if (error) {
      console.error(`Erro ao inserir lote de produtos (${i} a ${i + batch.length - 1}):`, error);
      errorCount += batch.length;
    } else {
      successCount += batch.length;
      console.log(`✅ Lote de produtos (${i} a ${i + batch.length - 1}) inserido com sucesso.`);
    }
...
async function populateProducts() {
  console.log('=== Script de População de Produtos para Supabase (Versão Corrigida) ===\n');
  ...
  try {
    ...
    try {
      ...
      console.log('Como o script não pode receber input, vamos pular esta etapa. Use o SQL Editor para limpar a tabela se necessário.');
    ...
    // Popule os produtos por categoria
    let totalProducts = 0;
    let totalSuccess = 0;
    let totalErrors = 0;
    ...
    for (const categoryId in productsByCategory) {
      ...
      // Inserir produtos em lotes pequenos
      const { successCount, errorCount } = await insertProductsBatch(products, categoryId);
    ...
    console.log(`Inseridos com sucesso: ${totalSuccess}`);
    console.log(`Falhas: ${totalErrors}`);

    if (totalSuccess === totalProducts) {
